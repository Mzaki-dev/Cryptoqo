    ...
    <footer className="border-t border-border bg-background">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="py-14 grid grid-cols-1 md:grid-cols-5 gap-10">
          ...
          <div className="md:col-span-2 space-y-4">
            <Link to="/" className="inline-flex items-center gap-2.5">
              ...
              <span className="font-semibold text-base tracking-tight">Crypto<span className="text-primary">Qo</span></span>
            </Link>
            <p className="text-sm text-muted-foreground leading-relaxed max-w-xs">
              CryptoQo empowers traders to transform raw market data into clear, actionable insights — making crypto easier to understand and act on.
            </p>
            {/* Social icons */}
            <div className="flex items-center gap-3 pt-1">
              {[
                { icon: TwitterX, href: '#', label: 'X', color: 'text-[#1DA1F2]' },
                { icon: Instagram, href: '#', label: 'Instagram', color: 'text-pink-500' },
                { icon: LinkedIn, href: '#', label: 'LinkedIn', color: 'text-[#0A66C2]' },
                { icon: GitHub, href: '#', label: 'GitHub', color: 'text-black dark:text-white' },
              ].map(({ icon: Icon, href, label, color }) => (
                <a
                  key={label}
                  href={href}
                  aria-label={label}
                  className={`p-2 rounded-md ${color} hover:opacity-80 transition-opacity`}
                >
                  <Icon />
                </a>
              ))}
        ...
        <div className="border-t border-border py-6 flex flex-col sm:flex-row justify-between items-center gap-3">
          <p className="text-sm text-muted-foreground">&copy; {year} CryptoQo. All rights reserved.</p>
          <div className="flex items-center gap-5">
            {['Privacy Policy', 'Terms of Service', 'Cookie Settings'].map((item) => (
              <a key={item} href="#" className="text-sm text-muted-foreground underline-offset-4 hover:underline transition-colors">
                {item}
              </a>
            ))}
          </div>
